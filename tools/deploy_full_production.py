...
def read_local_targets() -> tuple[dict[str, bytes], dict]:
    ...
    if manifest.get("file_count") != 23:
        warn(f"SEO manifest file_count is {manifest.get('file_count')}, expected 23.")
    ...
    ordered = dict(
        sorted(
            targets.items(),
            key=lambda item: (
                item[0] in ("app/Config/routes.php", "app/webroot/.htaccess", ".htaccess"),
